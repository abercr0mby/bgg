...
function getProductEntity(req, res, callback){
	...
	cds.$importEntities(
		[{ $entity: "bgg.db::game.product", $fields: { categories: { $association: { $lazy: true } } } },{ $entity: "bgg.db::game.category", $fields: { categorizes: { $association: { $lazy: true } } } }],
		importCallback
	);
}
...
function getProductInstance(req, res, callback){
	...
	getProductEntity(req, res, function(productEntity, categoryEntity){
		function transactionCallback(err, tx) {
			tx.$get(productEntity, { id: Number(req.params.productId) }, callback);
		}		
...
function createProduct(req, res){
	...
	getProductEntity(req, res, function(productEntity, categoryEntity){
		function transactionCallback(transError, tx) {
			    tx.$find(categoryEntity, { name: req.params.catName }, function (findError, cats) {
